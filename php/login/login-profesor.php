...
if (isset($_POST)) {
    if (!empty($_POST["correoProfesor"]) AND !empty($_POST["claveProfesor"])) {
        ...
        if (validarCorreo($_POST["correoProfesor"]) AND validarPassword($_POST["claveProfesor"])) {
            ...
            $stmt = $conexion -> prepare("SELECT COUNT(*) FROM info_personal WHERE correo = ?");
            ...
            if ($stmt -> num_rows() > 0) {
                ...
                $login = $conexion -> prepare("SELECT cvePersona, contra FROM profesor NATURAL JOIN personal_escolar NATURAL JOIN info_personal WHERE correo = ?");
